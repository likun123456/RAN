	...
	<script type="text/javascript">
			...
			$("#searchForm").validate({
				submitHandler: function(form){
					if ($("#netId option:selected").val()==""){
	                	...
	                	if($("#searchForm").attr('action')=="/epc/a/cheat/cheatFreeBusiness/exportExcel"){
	                		form.submit();
		...
		function showChart(data){			
	    	...
	    	for ( var key in data) {
				...
				for(var n=0;n<data[key].listCheatFreeBusiness.length;n++){
					result.push(Number(data[key].listCheatFreeBusiness[n].total));
					if(dateTimeRemark==0){
		...
		function queryChart(){
			$.post('${ctx}/cheat/cheatFreeBusiness/findChart', {
		        "netId": $("#netId option:selected").val(),
		        "startTime":$("#startTime").val(),
		        "endTime":$("#endTime").val(),
		        "gran":$("#gran option:selected").val(),
		        "ratingGroup":$("#ratingGroupId").val()
		    }, function(dataQuery) {
		    	showChart(dataQuery);
		    });
		}
		...
		function queryTable(){
			...
			$('#table').bootstrapTable({
				url:'${ctx}/cheat/cheatFreeBusiness/queryList',
		        method:'POST',
		        dataType:'json',
		        cache:false,
		        ...
		        onLoadSuccess: function (data) {
		        	...
		        	queryChart();
		        	$("#chart_outside_div ul li").eq(1).html("<b>按"+$('#gran option:selected').text()+"统计(KB)</b>");
		        	$("#chart_outside_div ul li").eq(2).html("<b>统计时段："+startTime+"-"+endTime+"</b>");
		        	$("#table_outside_div ul li").eq(1).html("<b>按"+$('#gran option:selected').text()+"统计(KB)</b>");
...
<body>
	<form:form id="searchForm" name="searchForm" modelAttribute="tCheatFreeBusiness" class="breadcrumb form-search" action="">
		<ul class="ul-form">
		    <li>
		        <label>网元</label>
		        <form:select path="netId" class="input-xlarge" style="width:150px;" id="netId">
			...
			<li>
		        <label>业务类型</label>
		         <sys:treeselect id="ratingGroup" name="ratingGroup" cssStyle="width:100px;" value="${ratingGroup}" labelName="ratingGroup" labelValue="${ratingGroup}"
					title="业务类型" url="/cheat/cheatFreeBusiness/treeData" notAllowSelectRoot="true" notAllowSelectParent="true" checked="true" />
			</li>
			<li>
			   <label>粒度</label>
		       <form:select path="gran" class="input-xlarge" style="width:100px;" id="gran">
		             <form:option value="" label="请选择"/>
			...
			<li><label>时间段</label>
		       <form:select id="pastScope" path="temp_field1" class="input-medium" style="width:100px;">
                   <form:option value="" label="请选择"/>           
			...
			<li id="startTimeLi">	
				<label>开始时间</label>
				<form:input id="startTime" path="temp_field2" type="text" readonly="readonly" maxlength="20" class="txt Wdate" style="width:150px;"
				 onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:00',isShowClear:true});"/>
			</li> 
			<li id="endTimeLi">
				<label>结束时间</label>
				<form:input id="endTime" path="temp_field3" type="text" readonly="readonly" maxlength="20" class="txt Wdate" style="width:150px;"
				 onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:00',isShowClear:true});"/>
			</li>
			<li class="btns"><input id="queryBtn" class="btn-new btn-search" value="查询" type="submit"/></li>
			<li class="btns"><input id="exportBtn" class="btn-new btn-save" value="导出" type="submit" onclick="searchForm.action='${ctx}/cheat/cheatFreeBusiness/exportExcel';"/></li>
		</ul>
	</form:form>

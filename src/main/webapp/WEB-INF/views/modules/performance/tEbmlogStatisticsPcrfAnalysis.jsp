...
<head>
	...
	<script type="text/javascript">
		...
		function openEbmSingleSearchWindow(value,key){
			...
			}else if(key=="RX_SESSION_ID"){
				url = "${ctx}/userquery/tEbmSingleSearch?rx="+value+"&isImsi="+isImsi+"&startTime="+startTime+"&endTime="+endTime;
			}else{
				url = "${ctx}/userquery/tEbmSingleSearch?gx="+value+"&isImsi="+isImsi+"&startTime="+startTime+"&endTime="+endTime;
			}
			openMaxWindow(url);
		}
		
		function exportTable(){
			window.location.href= "${ctx}/performance/tEbmlogStatistics/exportAnalysisExcel.do"
		}
	</script>
</head>
<body>
									...
									<colgroup>
							           <col style="width:auto;" />
										...
										<c:forEach items="${map.value}" var="o">
										    <tr>
										        <c:choose>
											        <c:when test="${map.key=='RX_SESSION_ID'||map.key=='GX_SESSION_ID'||map.key=='MSISDN'}">
											           <td style="padding:5;">
											               <c:choose>
											                   <c:when test="${o.obj!='undefined'}">
											                      <a href="javascript:void(0);"  onclick="openEbmSingleSearchWindow('${o.obj}','${map.key}')" style="cursor: pointer;">
												                     <span title="${o.obj}"><c:out value="${fns:abbr(o.obj,35)}"></c:out></span>
												                  </a>
											                   </c:when>
											                   <c:otherwise>
											        ...
											        <c:otherwise>
											           <td style="padding:5;"><span title="${o.obj}"><c:out value="${fns:abbr(o.obj,35)}"></c:out></td>
											        </c:otherwise>
											    </c:choose>

	...
	<script type="text/javascript">
		...
		function showChart(data){
			...
			$("#chart_div").highcharts({
		        ...
		        plotOptions: {
		            column: {
		                ...
		                events: {
				            click: function(e) {
							    var tac = e.point.category;
							    $("#tac").val(tac);
							    openNewWindow('${ctx}/performance/tTacSuccessRate/showEbmPieListWindow',1200,800);
						    }
						}
		            }
		        },
		    ...
		    for(var i=0;i<len;i++){
		    	...
		    	tac[i] = chartList[i].tac;
		    }
		    $("#chart_div").highcharts().addSeries({name:"TAC",data:success_rate});
		    $("#chart_div").highcharts().xAxis[0].setCategories(tac);
		}
		
		function queryTable(){
			$('#table').bootstrapTable("destroy");
			$('#table').bootstrapTable({
				url:'${ctx}/performance/tTacSuccessRate/queryList',
		        method:'POST',
		        dataType:'json',
		        cache:false,
		        ...
		        onLoadSuccess: function (data) {
		        	...
		        	showChart(data.rows);
		        	$("#chart_outside_div ul li").eq(1).html("<b>"+$('#eventType option:selected').text()+"成功率统计(TAC维度)</b>");
		        	$("#chart_outside_div ul li").eq(2).html("<b>统计时段："+startTime+"-"+endTime+"</b>");
		        	$("#table_outside_div ul li").eq(1).html("<b>"+$('#eventType option:selected').text()+"成功率统计(TAC维度)</b>");
...
<body>
    <input type="hidden" value="" id="tac"/>
	<form:form id="searchForm" name="searchForm" modelAttribute="tTacSuccessRate" class="breadcrumb form-search" action="">
		<ul class="ul-form">
		    <li>
		        <label>查询网元</label>

		...
		<div class="accordion-group">
		    <div class="accordion-heading">
		    	<a class="accordion-toggle" data-toggle="collapse" data-parent="#menu-${param.parentId}" data-href="#collapse-${menu.id}" href="#collapse-${menu.id}" title="${menu.remarks}">
		    		...
		    		<%-- <i class="${not empty menu.icon ? menu.icon : 'circle-arrow-right'}"></i> --%>
		    		<span class="${menu.icon}" style="float:left; margin-top:-5px;font-size: 30px; "></span>
		    		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${menu.name}
		    	</a>
		    </div>
		    <div id="collapse-${menu.id}" class="accordion-body collapse ${not empty firstMenu && firstMenu ? 'in' : ''}">
				<div class="accordion-inner">
					<ul class="nav nav-list"><c:forEach items="${menuList}" var="menu2"><c:if test="${menu2.parent.id eq menu.id&&menu2.isShow eq '1'}">
						<li><a data-href=".menu3-${menu2.id}" href="${fn:indexOf(menu2.href, '://') eq -1 ? ctx : ''}${not empty menu2.href ? menu2.href : '/404'}" target="${not empty menu2.target ? menu2.target : 'mainFrame'}" ><%-- <i class="${not empty menu2.icon ? menu2.icon : 'circle-arrow-right'}"></i> --%><span class="${menu2.icon}" style="float:left; margin-top:-3px;font-size: 28px"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${menu2.name}</a>
							<ul class="nav nav-list hide" style="margin:0;padding-right:0;"><c:forEach items="${menuList}" var="menu3"><c:if test="${menu3.parent.id eq menu2.id&&menu3.isShow eq '1'}">
								<li class="menu3-${menu2.id} hide"><a href="${fn:indexOf(menu3.href, '://') eq -1 ? ctx : ''}${not empty menu3.href ? menu3.href : '/404'}" target="${not empty menu3.target ? menu3.target : 'mainFrame'}" ><%-- <i class="${not empty menu3.icon ? menu3.icon : 'circle-arrow-right'}"></i> --%><span class="${menu3.icon}" style="float:left; margin-top:-3px;font-size: 28px;"></span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${menu3.name}</a></li></c:if>

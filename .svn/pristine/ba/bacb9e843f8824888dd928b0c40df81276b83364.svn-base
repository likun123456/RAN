...
$.extend($.fn, {
    initJerichoTab: function(setting) {
        ...
        function createJerichoTab() {
            ...
            */
            var jerichotab = $('<div id="'+opts.uniqueId+'" class="jericho_tab"><div class="tab_content"><div id="jerichotab_contentholder" class="content" /></div><div class="tab_pages" style="position: relative; bottom:-46px;left:-28px;"><div class="tabs"><ul /></div></div></div>')
                ...
                addTab: function(tabsetting) {
                    ...
                    */
                    ...
                    var newTab = $('<li name="'+ps.name+'" class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                            '<div class="tab_left"  style="width:' + (opts.tabWidth - 20) + 'px"  >' +
                                (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                                '<div class="tab_text" title="' + t + '"  style="width:' + (opts.tabWidth - 60 + (ps.iconImg == '' ? 25 : 0)) + 'px"  >' + t.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                                '<div class="tab_close">' + (ps.closeable ? '<a href="javascript:" title="关闭">&nbsp;</a>' : '') + '</div>' +
                            '</div>' +
                            '<div class="tab_right">&nbsp;</div>' +
                        '</li>').appendTo($.fn.jerichoTab.tabpage).css('opacity', '0').applyHover()
	                          .applyCloseEvent().animate({ 'opacity': '1', width: opts.tabWidth-15 }, 100, function() {
	                    $.fn.setTabActive(curIndex);
	                });
                    //use an Array named "tabHash" to restore the tab information
                    var firstLevel = $('.menu.active a span:eq(1)').html();
                    var secondLevel = $('.accordion-heading.accordion-heading1 a').text();
                	//var thirdLevel = $('.jericho_tab .tab_selected .tab_text').html();
                    var thirdLevel = ps.title;
                    ...
                    tabHash.push({
                        ...
                        dataNav: firstLevel + "," + secondLevel + "," + thirdLevel,
                        //dataNav: secondLevel + " > " + thirdLevel,
    ...
    loadData: function(flag) {
        return this.each(function() {
        	...
        	//var first = $('.menu.active a span').html();
        	//var second = $('.jericho_tab .tab_selected .tab_text').html();
        	//$('#navdiv').html(first + ' > ' + second);
            ...
            for (var tab in tabHash) {
                if (tabHash[tab].tabId == $(this).attr('id')) {
                    ...
                    dataNav = tabHash[tab].dataNav;
                    var nav = dataNav.split(",");
                    var curWwwPath = window.document.location.href;
                    var pathName = window.document.location.pathname;
                    var pos = curWwwPath.indexOf(pathName);
                    var localhostPaht = curWwwPath.substring(0, pos);
                    var projectName = pathName.substring(0, pathName.substr(1).indexOf('/') + 1);
                    var webPath = localhostPaht + projectName + "/static";
                   
                    
                    
                    var firstL = 	'<div style=" float:left;">'
				    				+	 '<table  border="0">'
				    				+      '<tr>'
				    				+			'<td style="background:url('+webPath+'/jerichotab/images/firstlevel_left.png);" width="4" height="30"></td><td style="background:url('+webPath+'/jerichotab/images/firstlevel_center.png);"><span style="font-size: 11px; font-weight: bold;color: black; margin: 0 20px 0 20px">'+nav[0]+'</span></td><td style="background:url('+webPath+'/jerichotab/images/firstlevel_right.png);" width="15"></td>'
				    				+      '</tr>'
				    				+	 '</table>'
				    				+  '</div>';
                    var secondL =   '<div style=" float:left;  margin-left:-5px">'
				    				+	 '<table  border="0">'
				    				+      '<tr>'
				    				+			'<td style="background:url('+webPath+'/jerichotab/images/secondlevel_left.png);" width="11" height="30"></td><td style="background:url('+webPath+'/jerichotab/images/secondlevel_center.png);"><span style="font-size: 11px; font-weight: bold;color: black; margin: 0 20px 0 15px">'+nav[1]+'</span></td><td style="background:url('+webPath+'/jerichotab/images/secondlevel_rigth.png);" width="17"></td>'
				    				+      '</tr>'
				    				+	 '</table>'
				    				+  '</div>';
				    var thirdL = 	'<div style=" float:left;  margin-left:-5px">'
				    				+	 '<table  border="0">'
				    				+      '<tr>'
				    				+			'<td style="background:url('+webPath+'/jerichotab/images/thirdlevel_left.png);" width="11" height="30"></td><td style="background:url('+webPath+'/jerichotab/images/thirdlevel_center.png);"><span style="font-size: 11px; font-weight: bold;color: black; margin: 0 20px 0 10px">'+nav[2]+'</span></td><td style="background:url('+webPath+'/jerichotab/images/thirdlevel_right.png);" width="17"></td>'
				    				+      '</tr>'
				    				+	 '</table>'
				    				+  '</div>';
				    
                    var navStr = '<table border="0">'
				        				+'<tr>'
				        				+ '<td>'
				        				+ firstL
				        				+ secondL

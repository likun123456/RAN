...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#endTimeLi").hide();
			var netId = '26';
			var startTime = '2018-05-01 09:00:00';
			var endTime = '2018-05-01 11:00:00';
			...
			$('#table').bootstrapTable({
		      ...
		      /*   }, {
		            ...
		            formatter:function(value,row,index){
						...
						startTime = $("#startTime").val();
						endTime = $("#endTime").val();
						if((cc=="0"&&(scc=="2"||scc=="0"))||(cc=="2"&&scc=="2")){
							return "";
						}else{
		            ...
		            valign : 'middle',
		            formatter:function(value,row,index){
		            	var cc = parseInt(row.causecode.split("(")[0]);
		            	var scc = parseInt(row.subcausecode.split("(")[0]);
		...
		function showChart(data){
			$('#chart_div').highcharts({
		        ...
		        tooltip: {
		        	formatter: function() {
		        		...
		        		var cc = o.split(" ")[0];
		        		var scc = o.split(" ")[1];
		        		return cc+"  ("+this.point.name.split("|")[1]+" "+scc+" ("+this.point.name.split("|")[2]+" "+this.point.name.split("|")[5]+' 所占比例:<b>'+this.point.percentage.toFixed(2)+'%</b>';
		        	} 
		        },
		        plotOptions: {
		            pie: {
				...
				window.frames['ebmlogFrame'].document.location	= '${ctx}/performance/tEbmlogStatistics/analysis?cc='+cc+'&scc='+scc+'&netid='+netid+'&startTime='+startTime+
	                               '&endTime='+endTime+'&eventType='+eventType+'&ccdes='+encodeURIComponent(ccdes)+'&sccdes='+encodeURIComponent(sccdes)+
	                               '&action='+action+'&domain='+domain+ '&statisticType='+"value";
								 switchTab(2);
	        ...
	        for(var i=0;i<len;i++){
	        	...
	        	arr[i][0] = "CC:" + chartList[i].causecode.split("(")[0] + " SCC:" + chartList[i].subcausecode.split("(")[0] + 
	        			    ...
	        			    "|" + chartList[i].cc + "|" + chartList[i].scc + "|"+chartList[i].failures+"次";
...
<body>
    <ul class="nav nav-tabs">
		<li class="active" id="menu1" onclick="setTab('menu',1,3)"><a>MME错误码多维分析</a></li>

	...
	<script type="text/javascript">
			...
			$("#searchForm").validate({
				submitHandler: function(form){
					if ($("#netId option:selected").val()==""){
	                	...
	                	if($("#searchForm").attr('action')=="/epc/a/cheat/cheatType/exportExcel"){
	                		form.submit();
		...
		function showChart(data){			
	    	...
	    	for ( var key in data) {
				...
				for(var n=0;n<data[key].listCheatType.length;n++){
					result.push(Number(data[key].listCheatType[n].freetotal));
					if(dateTimeRemark==0){
		...
		function queryChart(){
			$.post('${ctx}/cheat/cheatType/findChart', {
		...
		function queryTable(){
			...
			$('#table').bootstrapTable({
				url:'${ctx}/cheat/cheatType/queryList',
		        method:'POST',
		        dataType:'json',
		        cache:false,
...
<body>
	<form:form id="searchForm" name="searchForm" modelAttribute="tCheatType" class="breadcrumb form-search" action="">
		<ul class="ul-form">
			...
			<li class="btns"><input id="exportBtn" class="btn-new btn-save" value="导出" type="submit" onclick="searchForm.action='${ctx}/cheat/cheatType/exportExcel';"/></li>
		</ul>
	</form:form>

...
<head>
	...
	<script type="text/javascript">
				                    ...
				                    click: function(e) {  
				                    	    ...
				                    	    if(pn.indexOf('MME') > -1 ) {
				                    	    	for(var i=0;i<chartlist.length;i++){
					                    	    	var _temp = chartlist[i][0].kpi;
					    			                for(var t=0;t<formula_ids.length;t++){
					    			                	if(formula_ids[t]==_temp){
					    			                		var formula_name = "X:"+kpinames[t];
					    			                	}
					    			                }
				                    	    	}
		...
		function showDetail(netType,netId,formulaId,recordTime,netName,formulaName,formulaText) {
			top.$.jBox.open("iframe:${ctx}/performance/multipleIndex/showDetail?netType="+netType+"&netId="+netId+"&formulaId="+formulaId+"&recordTime="+recordTime+"&netName="+encodeURIComponent(netName)+"&formulaName="+encodeURIComponent(formulaName)+"&formulaText="+encodeURIComponent(formulaText), "查看明细",1200,600,{
...
<body>
	<div class="navbar navbar-default navbar-fixed-top">
      <div class="navbar-inner">
        ...
        <div style="float:right" class="form-inline">
        	...
        	<span class="icon-h"  style="font-size: 30px; margin-top: -5px;cursor:pointer;margin-right: 15px" onclick="queryChartData()"></span><p style="cursor:pointer;display:inline-block;font-size: 15px; margin-right: 10px;margin-top: 10px" onclick="queryChartData()">图表模式</p>
	...
	<form:form id="searchForm" modelAttribute="tMultipleIndex" action="${ctx}/performance/multipleIndex/exportExcel" method="post" class="breadcrumb form-search" style="margin-top:5px;">
		<div style="margin-left:14px;">
			...
			网元名称:
                <sys:treeselect id="netid" name="netid" cssStyle="width:100px;" value="${netid}" labelName="netid" labelValue="${netid}"
					title="网元名称" url="/performance/multipleIndex/treeData" notAllowSelectRoot="true" notAllowSelectParent="true" checked="true" />
			
			统计指标:
				<sys:treeselect id="formulaType" name="formulaType" cssStyle="width:100px;" value="${formulaType}" labelName="formulaType" labelValue="${formulaType}"
			...
			<div class="pull-right">
				<input id="btnSubmit" class="btn-new btn-search" type="button" onclick="queryChartData()" value="查询"/>

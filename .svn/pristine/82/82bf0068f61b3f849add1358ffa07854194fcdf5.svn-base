		    ...
		    <div class="panel-body" id="chart_panel" >
				...
				<c:forEach items="${pieMap}" var="map">
					...
					<table name="table" class="table table-striped table-bordered table-condensed">
				        ...
				        <c:choose>
							<c:when test="${map.key=='ECI'||map.key=='TAC'}">
							    <thead>
									<tr>
									    <th style="width: 25%">${fns:getDictLabel(map.key,'biz_ebmlog_analysis',map.key)}</th>
									    <th style="width: 25%">失败次数</th>
									    <th style="width: 25%">总数</th>
											...
											<colgroup>
									           <col style="width:auto;" />
												...
												<c:forEach items="${map.value}" var="o">
												    <tr>
													    <td style="width: 26%"><c:out value="${o.obj}"></c:out></td>
													    <td style="width: 26%"><c:out value="${o.failures}"></c:out></td>
													    <td style="width: 26%"><c:out value="${o.totalCount}"></c:out></td>
													    <td style="width: 22%"><c:out value="${o.successRate}"></c:out></td>
													</tr>
										        </c:forEach>
												</tbody>
											</table>

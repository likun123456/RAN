...
<html>
<head>
	<title>当前用户IMSI对应业务编码流量分布饼形图</title>
		  ...
		  $('#pie1').highcharts({
		            ...
		            data: [
		            	<c:forEach items="${list}" var="temp" varStatus="st1">
				            <c:if test="#st1.index!=0">
								...
								['业务编码：${temp.ratingGroup}<br/>'+'业务名称：${temp.ratingGroupName} <br/>'+
								 '总流量：<fmt:formatNumber type="number" value="${(temp.total)/1024}" pattern="0.00" maxFractionDigits="2"/> KB<br/>',${temp.total}]	
		            	</c:forEach>
		            ]
		        }]
		    });
		   $('#pie2').highcharts({
		        ...
		        series: [{
		            ...
		            name: 'ratingGroup',
		            data: [
		            	['免费流量：${freeTotal} KB <br/> 百分比',  ${freeTotal}],

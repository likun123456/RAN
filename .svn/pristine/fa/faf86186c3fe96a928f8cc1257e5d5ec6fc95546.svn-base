...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			// 用户-菜单
			var zNodes=[
					{id:"0", pId:"-1", name:"所有OSS"},
					<c:forEach items="${netelementList}" var="menu">
						{id:"${menu.id}", pId:"0", name:"${not empty menu.temp_field1?menu.temp_field2:menu.fname}"},
		            </c:forEach>];
			// 初始化树结构
			...
			var rootNode = tree.getNodeByParam("id", "0");
...
<body>
	...
	<form:form id="inputForm" modelAttribute="tCollectordeploy" action="${ctx}/collectset/elementcollect/saveOss" method="post" class="form-horizontal">
		<form:hidden path="id"/>
